...
void reconnect()
{
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("EckkellerESP")) {
...
void setup()
{
 ...
 // digitalWrite(D4, HIGH);  // LED ist aus
   ...
   */
    ...
    client.publish("Sensoren/Eckkeller/RSSI", msgrssi); 
    delay(50); 
    client.publish("Sensoren/Eckkeller/Luft", msgluft);
    delay(50);
    client.publish("Sensoren/Eckkeller/Spg", msgspg);
    delay(50);
    client.publish("Sensoren/Eckkeller/Temp", msgtemp);
    delay(50);
    } /*
    else
    {
